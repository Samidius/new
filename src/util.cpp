...
#else
    const char *pszModule = "MorozzzCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir() {
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MorozzzCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MorozzzCoin
    // Mac: ~/Library/Application Support/MorozzzCoin
    // Unix: ~/.MorozzzCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MorozzzCoin";
#else
    fs::path pathRet;
...
boost::filesystem::path GetConfigFile() {
    boost::filesystem::path pathConfigFile(GetArg("-conf", "MorozzzCoin.conf"));
...
boost::filesystem::path GetPidFile() {
    boost::filesystem::path pathPidFile(GetArg("-pid", "MorozzzCoind.pid"));
        ...
        if (abs64(nMedian) < 70 * 60) {
            ...
            if (!fDone) {
                ...
                if (!fMatch) {
                    fDone = true;
                    string strMessage = _(
                            "Warning: Please check that your computer's date and time are correct! If your clock is wrong MorozzzCoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage + " ", string("MorozzzCoin"),
