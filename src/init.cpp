...
void Shutdown(void *parg) {
    ...
    RenameThread("MorozzzCoin-shutoff");
...
bool AppInit(int argc, char *argv[]) {
    ...
    try {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help")) {
            ...
            std::string strUsage = _("MorozzzCoin version") + " " + FormatFullVersion() + "\n\n" +
                                   _("Usage:") + "\n" +
                                   "  MorozzzCoind [options]                     " + "\n" +
                                   "  MorozzzCoind [options] <command> [params]  " + _("Send command to -server or MorozzzCoind") +
                                   "\n" +
                                   "  MorozzzCoind [options] help                " + _("List commands") + "\n" +
                                   "  MorozzzCoind [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "MorozzzCoin:"))
                fCommandLine = true;
...
bool static InitError(const std::string &str) {
    uiInterface.ThreadSafeMessageBox(str, _("MorozzzCoin"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str) {
    uiInterface.ThreadSafeMessageBox(str, _("MorozzzCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION |
...
std::string HelpMessage() {
    string strUsage = _("Options:") + "\n" +
                      "  -?                     " + _("This help message") + "\n" +
                      "  -conf=<file>           " + _("Specify configuration file (default: MorozzzCoin.conf)") + "\n" +
                      "  -pid=<file>            " + _("Specify pid file (default: MorozzzCoind.pid)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  MorozzzCoin is probably already running."),
                                   strDataDir.c_str()));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("MorozzzCoin version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "MorozzzCoin server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName)) {
        ...
        if (r == CDBEnv::RECOVER_OK) {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("MorozzzCoin"),
    ...
    if (nLoadWalletRet != DB_LOAD_OK) {
        if (nLoadWalletRet == DB_CORRUPT)
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of MorozzzCoin") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE) {
            strErrors << _("Wallet needed to be rewritten: restart MorozzzCoin to complete") << "\n";
