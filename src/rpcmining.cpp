...
Value getworkex(const Array &params, bool fHelp) {
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "MorozzzCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "MorozzzCoin is downloading blocks...");
...
Value getwork(const Array &params, bool fHelp) {
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "MorozzzCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "MorozzzCoin is downloading blocks...");
